...
const THEMES = [
    {
        ...
        cardBg: '#ffffff', cardBorder: 'rgba(99,102,241,0.12)',
        textColor: '#1e1b4b', subtextColor: '#6b7280', mutedColor: '#9ca3af', accent: '#6366f1',
        accentGrad: 'linear-gradient(135deg, #6366f1, #8b5cf6)',
        inputBg: '#f3f4f8', inputBorder: '#e5e7eb',
        previewBg: '#f2f4f8', previewHeader: '#f7f8fc',
        ownBubble: 'linear-gradient(135deg, #6366f1, #8b5cf6)', ownText: '#ffffff',
        otherBubble: '#e8e9f4', otherText: '#374151',
        btnBack: '#f3f4f6', btnBackText: '#374151', btnBackBorder: '#e5e7eb',
    },
    {
        id: 'dark', label: 'Тёмная', icon: '🌙',
        pageBg: 'linear-gradient(135deg, #0f0c29 0%, #1a1630 60%, #0d0b1a 100%)',
        cardBg: '#1a1a2e', cardBorder: 'rgba(99,102,241,0.18)',
        textColor: '#e2e8f0', subtextColor: '#7c7caa', mutedColor: '#4a4a7a', accent: '#818cf8',
        accentGrad: 'linear-gradient(135deg, #6366f1, #8b5cf6)',
        inputBg: '#252538', inputBorder: 'rgba(99,102,241,0.2)',
        previewBg: '#16162a', previewHeader: '#13131f',
        ownBubble: 'linear-gradient(135deg, #6366f1, #8b5cf6)', ownText: '#ffffff',
        otherBubble: '#2a2a3d', otherText: '#c0c0d8',
        btnBack: '#252538', btnBackText: '#a5b4fc', btnBackBorder: 'rgba(99,102,241,0.2)',
    },
    {
        id: 'oled', label: 'OLED', icon: '✦',
        pageBg: '#000000',
        cardBg: '#050508', cardBorder: 'rgba(167,139,250,0.15)',
        textColor: '#e2e8f0', subtextColor: '#6b5fa0', mutedColor: '#3a3a5a', accent: '#a78bfa',
        accentGrad: 'linear-gradient(135deg, #7c3aed, #a78bfa)',
        inputBg: '#0d0d18', inputBorder: 'rgba(167,139,250,0.15)',
        previewBg: '#000000', previewHeader: '#000000',
        ownBubble: 'linear-gradient(135deg, #5b21b6, #a78bfa)', ownText: '#ffffff',
        otherBubble: '#0d0d18', otherText: '#c4b5fd',
        btnBack: '#0d0d18', btnBackText: '#c4b5fd', btnBackBorder: 'rgba(167,139,250,0.15)',
    },
];
...
const SetupModal: React.FC<SetupModalProps> = ({ token, onComplete }) => {
    ...
    const inputStyle: React.CSSProperties = {
        width: '100%', padding: '13px 14px', border: `1.5px solid ${isStep2 ? theme.inputBorder : '#e5e7eb'}`,
        borderRadius: 10, outline: 'none', fontSize: 15, backgroundColor: isStep2 ? theme.inputBg : '#fafafa',
        color: isStep2 ? theme.textColor : '#1e1b4b', boxSizing: 'border-box', fontFamily: 'inherit', transition: T,
    };
    ...
    return (
        <div style={{
            ...
            >
                ...
                <p style={{ fontSize: 13, color: subtextColor, marginBottom: isMobile ? 14 : 20, transition: T, textAlign: 'center' }}>Настройте свой профиль</p>

                {/* Steps indicator */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: isMobile ? 16 : 20 }}>
                    {[1, 2].map((n, i) => (
                        <React.Fragment key={n}>
                            {i > 0 && <div style={{ width: 36, height: 2, borderRadius: 1, background: step >= 2 ? theme.accentGrad : (isStep2 ? theme.inputBorder : '#e5e7eb'), transition: T }} />}
                            <div style={{
                ...
                {/* ── STEP 1 ── */}
                {step === 1 && (<>
                    <p style={{ fontSize: 12, color: '#6b7280', marginBottom: 12, alignSelf: 'flex-start' }}>Шаг 1 — Имя и тег</p>
                    ...
                    <div style={{ width: '100%', marginBottom: 12 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #e5e7eb', borderRadius: 10, backgroundColor: '#fafafa', overflow: 'hidden' }}>
                            <span style={{ padding: '13px 8px 13px 14px', fontSize: 15, fontWeight: 700, color: '#6366f1', backgroundColor: '#f3f4ff', borderRight: '1px solid #e5e7eb', flexShrink: 0 }}>@</span>
                            <input type="text" placeholder="например: ivan_petrov" value={tag}
                                onChange={e => setTag(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 30))}
                                style={{ width: '100%', padding: '13px 14px', border: 'none', outline: 'none', fontSize: 15, backgroundColor: '#fafafa', fontFamily: 'inherit', color: '#1e1b4b' }}
                                autoFocus={!isMobile} />
                        </div>
                    ...
                    <div style={{ width: '100%', marginBottom: 12 }}>
                        ...
                        <input type="text" placeholder="Как вас называть?" value={username}
                ...
                {/* ── STEP 2 ── */}
                {step === 2 && (<>
                    <p style={{ fontSize: 12, color: subtextColor, marginBottom: 12, alignSelf: 'flex-start', transition: T }}>Шаг 2 — Тема оформления</p>

                    {/* Mini chat preview — hidden on very small screens to save space */}
                    {!isMobile && (
                        <div style={{ width: '100%', borderRadius: 14, overflow: 'hidden', border: `1px solid ${theme.inputBorder}`, marginBottom: 14, transition: T }}>
                            <div style={{ backgroundColor: theme.previewHeader, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${theme.inputBorder}`, transition: T }}>
                                <div style={{ width: 28, height: 28, borderRadius: '50%', background: theme.accentGrad, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: 'white', fontWeight: 700, flexShrink: 0 }}>A</div>
                                <div>
                            ...
                            <div style={{ backgroundColor: theme.previewBg, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 7, transition: T }}>
                                <div style={{ display: 'flex', gap: 7, alignItems: 'flex-end' }}>
                                    <div style={{ width: 22, height: 22, borderRadius: '50%', background: theme.accentGrad, flexShrink: 0 }} />
                                    <div style={{ backgroundColor: theme.otherBubble, color: theme.otherText, padding: '7px 11px', borderRadius: '13px 13px 13px 3px', fontSize: 12, lineHeight: 1.4, maxWidth: '75%', transition: T }}>Привет! Добро пожаловать 👋</div>
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                                    <div style={{ background: theme.ownBubble, color: theme.ownText, padding: '7px 11px', borderRadius: '13px 13px 3px 13px', fontSize: 12, lineHeight: 1.4, transition: T }}>Отлично выглядит! ✨</div>
                                    <div style={{ fontSize: 10, color: theme.mutedColor, marginTop: 2, transition: T }}>12:01 ✓✓</div>
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        {THEMES.map(th => {
                            ...
                            return (
                                <button key={th.id} onClick={() => setSelectedId(th.id)} style={{
                                    ...
                                    boxShadow: active ? `0 0 0 3px ${th.accent}28` : 'none',
                                    transform: active ? 'scale(1.04)' : 'scale(1)',
                                    transition: 'all 0.22s cubic-bezier(0.4,0,0.2,1)',
                                    position: 'relative', fontFamily: 'inherit',
                                }}>
                                    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 3, padding: '0 2px' }}>
                                        <div style={{ height: 5, width: '60%', borderRadius: 6, background: th.otherBubble }} />
                                        <div style={{ height: 5, width: '76%', borderRadius: 6, background: th.ownBubble, alignSelf: 'flex-end' }} />
                                        <div style={{ height: 5, width: '50%', borderRadius: 6, background: th.otherBubble }} />
                                    </div>
                                    <span style={{ fontSize: isMobile ? 16 : 18 }}>{th.icon}</span>
                                    <span style={{ fontSize: isMobile ? 10 : 11, fontWeight: 700, color: active ? th.accent : th.subtextColor, transition: 'color 0.2s' }}>{th.label}</span>
                                    {active && (
                                        <div style={{ position: 'absolute', top: 5, right: 5, width: 15, height: 15, borderRadius: '50%', background: th.accentGrad, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, width: '100%' }}>
                        <button onClick={() => setStep(1)} style={{ padding: isMobile ? '13px 14px' : '12px 18px', backgroundColor: theme.btnBack, color: theme.btnBackText, border: `1px solid ${theme.btnBackBorder}`, borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: T, fontFamily: 'inherit', flexShrink: 0, minHeight: 48 }}>
                            ← Назад
                        </button>
                        <button onClick={handleComplete} disabled={loading} style={{ flex: 1, padding: '13px', background: theme.accentGrad, color: 'white', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: loading ? 0.7 : 1, boxShadow: `0 4px 20px ${theme.accent}45`, transition: T, fontFamily: 'inherit', minHeight: 48 }}>
            ...
            {exiting && (
                <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: theme.pageBg, animation: 'setupExitFill 0.55s cubic-bezier(0.4,0,0.2,1) forwards', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <img src="/logo192.png" alt="Aurora" style={{ width: 72, height: 72, borderRadius: 18, boxShadow: `0 0 40px ${theme.accent}60`, animation: 'setupExitFill 0.55s 0.1s cubic-bezier(0.4,0,0.2,1) both' }} />
                </div>
            )}
        </div>
    );
