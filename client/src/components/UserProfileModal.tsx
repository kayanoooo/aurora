...
const UserProfileModal: React.FC<UserProfileModalProps> = ({
    ...
    const formatLastSeenProfile = (lastSeen: string): string => {
        try {
            ...
            if (diffH < 6) return `был(а) ${diffH} ч. назад`;
            ...
            if (msgDay.getTime() === today.getTime()) return `был(а) сегодня в ${hhmm}`;
            if (msgDay.getTime() === yesterday.getTime()) return `был(а) вчера в ${hhmm}`;
            return `был(а) ${Math.floor((now.getTime() - date.getTime()) / 86400000)} дн. назад`;
        } catch { return 'был(а) недавно'; }
    };

    const formatBirthday = (val?: string) => {
        if (!val) return null;
        try { return new Date(val).toLocaleDateString('ru-RU', { year: 'numeric', month: 'long', day: 'numeric' }); }
        catch { return val; }
    };
    ...
    return (
        ...
        <div style={t.overlay} className={closing ? 'modal-backdrop-exit' : 'modal-backdrop-enter'} onClick={close}>
            ...
            >
                {/* Profile panel */}
                <div style={{ width: 340, flexShrink: 0, padding: '32px 28px 24px', boxSizing: 'border-box', textAlign: 'center', overflowY: 'auto', maxHeight: '88vh' }}>
                        ...
                        {(isOnline ?? fullUser.is_online)
                            ...
                            : fullUser.last_seen && fullUser.last_seen !== 'hidden'
                                ...
                                : fullUser.last_seen === 'hidden'
                                    ? <p style={{ fontSize: 13, color: sub, margin: '0 0 4px' }}>был(а) недавно</p>
                                    : null
                        }
                    ...
                    <div style={t.infoCard}>
                        ...
                        {fullUser.created_at && (
                            <div style={{ ...t.infoRow, borderBottom: 'none' }}>
                                <span style={{ fontSize: 16, flexShrink: 0 }}>📅</span>
                ...
                <div style={{
                    ...
                    flexShrink: 0,
                    borderLeft: expanded ? `1px solid ${border}` : 'none',
                    display: 'flex',
                    ...
                    <div style={{ display: 'flex', borderBottom: `1px solid ${border}`, flexShrink: 0, alignItems: 'stretch' }}>
                        ...
                        <button onClick={() => setExpanded(false)} title="Свернуть"
        ...
        {avatarLightbox && avatarUrl && ReactDOM.createPortal(
            <div onClick={() => setAvatarLightbox(false)} style={{ position: 'fixed', inset: 0, zIndex: 9000, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out', backdropFilter: 'blur(8px)' }}>
...
const tokens = (dm: boolean) => ({
    ...
    infoCard: { backgroundColor: dm ? '#1e1e3a' : '#f5f3ff', borderRadius: 12, padding: '12px 16px', marginBottom: 20, textAlign: 'left' as const, border: dm ? '1px solid rgba(255,255,255,0.06)' : '1px solid #ede9fe' },
    infoRow: { display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: dm ? '1px solid rgba(255,255,255,0.06)' : '1px solid #ede9fe' },
});
